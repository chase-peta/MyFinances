...
namespace MyFinances.Models
{
    ...
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        {
            var entities = ChangeTracker.Entries().Where(x => x.Entity is BaseFinances && (x.State == EntityState.Added || x.State == EntityState.Modified));
            ...
            if (entities.Any())
            {
                foreach (var entity in entities)
                {
                    if (entity.State == EntityState.Added)
                    {
                        ...
                        if (((BaseFinances) entity.Entity).UserId == null)
                        {
                            ((BaseFinances) entity.Entity).UserId = HttpContext.Current.User.Identity.GetUserId();
                        }
                    }
                    else
                    {
